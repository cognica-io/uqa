...
class VersionedGraphStore:
    ...
    def __init__(self, base: GraphStore, *, graph_name: str) -> None:
        self._base = base
    ...
    def apply(self, delta: GraphDelta) -> int:
        ...
        for op in delta.ops:
            if op.kind == "add_vertex" and op.vertex is not None:
                self._base.add_vertex(op.vertex, graph=g)
                ...
                self._base.remove_vertex(op.vertex_id, graph=g)
                if existing is not None:
                    inverse.add_vertex(existing)
            elif op.kind == "add_edge" and op.edge is not None:
                self._base.add_edge(op.edge, graph=g)
                ...
                self._base.remove_edge(op.edge_id, graph=g)
