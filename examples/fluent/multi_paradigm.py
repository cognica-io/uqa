...
bought_together = [
    (1, 9, "also_bought"), (9, 1, "also_bought"),  # headphones <-> earbuds
    (3, 4, "also_bought"), (4, 3, "also_bought"),  # keyboard <-> mouse
    (5, 8, "also_bought"), (8, 5, "also_bought"),  # monitor <-> dock
    (5, 10, "also_bought"), (10, 5, "also_bought"),  # monitors together
    (3, 10, "also_bought"), (10, 3, "also_bought"),  # keyboard <-> gaming monitor
    (7, 8, "also_bought"), (8, 7, "also_bought"),  # webcam <-> dock
...
orders = [
    ...
    (3, {
        ...
        "items": [
            {"product_id": 5, "name": "ultrawide monitor", "price": 599.99, "qty": 1},
...
results = (
    text_q.and_(vec_q)
    .filter("price", LessThanOrEqual(250.0))
    .execute()
)
...
results = (
    engine.query(table="products")
    .traverse(1, "also_bought", max_hops=1)
    .execute()
)
for entry in results:
    if entry.doc_id != 1:
...
results = (
    engine.query(table="products")
    .traverse(3, "also_bought", max_hops=2)
    .execute()
)
for entry in sorted(results, key=lambda e: e.doc_id):
    if entry.doc_id != 3:
        v = gs.get_vertex(entry.doc_id)
        if v:
            print(f"  [{entry.doc_id}] {v.properties['name']} [{v.properties['category']}]")
...
text_signal = (
    engine.query(table="products")
    .term("monitor")
    .score_bayesian_bm25("monitor")
)
vec_signal = engine.query(table="products").knn(display_query, k=5)

fused = engine.query(table="products").fuse_log_odds(
    text_signal, vec_signal, alpha=0.6
)
...
results = (
    engine.query(table="orders")
    .path_aggregate("items.name", "count")
    .execute()
)
for entry in sorted(results, key=lambda e: e.doc_id):
    doc = engine._tables["orders"].document_store.get(entry.doc_id)
...
results = (
    engine.query(table="orders")
    .filter("items.name", Equals("webcam"))
    .execute()
)
for entry in results:
    doc = engine._tables["orders"].document_store.get(entry.doc_id)
...
for mode_name, fuse_fn in [
    ("log_odds", lambda: engine.query(table="products").fuse_log_odds(
        text_signal, vec_signal, alpha=0.5)),
    ("prob_and", lambda: engine.query(table="products").fuse_prob_and(
        text_signal, vec_signal)),
    ("prob_or", lambda: engine.query(table="products").fuse_prob_or(
        text_signal, vec_signal)),
]:
...
hybrid = (
    engine.query(table="products").term("wireless")
